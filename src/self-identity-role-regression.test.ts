...
async function testOwnerMemoryWinsOverAuthorizationRole(): Promise<void> {
  ...
  assert(!call.prompt.includes('配置展示名'), 'owner display metadata reached the final-answer prompt')
...
async function testOwnerWithoutMemoryIsUnknown(): Promise<void> {
  ...
  assert(!call.prompt.includes('OWNER'), 'OWNER authorization literal reached the final-answer prompt')
  assert(!call.prompt.includes('配置展示名'), 'owner display metadata reached the final-answer prompt')
