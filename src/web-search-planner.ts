...
当问题是一个现实世界的具体外部事实，而 Context / Authorized Memory 没有可靠证据，或模型知识可能过期、不完整、不确定时，即使它看起来像常识，也应优先 SEARCH。特别要认真考虑现实人物、主播、公司、游戏、产品、服务、排名、规模、价格、版本、当前状态、冷门实体，以及“X 是谁 / 最大的是谁 / 现在怎么样 / 哪个更大”这类可以由公开资料核实的问题。目标是降低无依据的“我不知道”或猜测，不是等用户把“搜索”两个字说出来。

不要把出现某几个名词或时间词作为唯一条件；要判断整个问题是否属于可公开验证的外部事实。数学、编程原理、稳定基础知识、群聊上下文、requester-local 信息、私人经历、当前 Authorized Memory 已足够的问题、无法通过公网得出的主观原因，以及只是闲聊，仍然必须 DIRECT。
